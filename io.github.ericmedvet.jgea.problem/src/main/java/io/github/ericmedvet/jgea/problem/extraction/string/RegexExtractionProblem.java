...
public record RegexExtractionProblem(
...
) implements ExtractionProblem<Character>,
    TargetEBProblem<Extractor<Character>,List<Character>,Set<IntRange>, ExtractionProblem.Outcome, SequencedMap<String, Double>> {
  ...
  public RegexExtractionProblem(
    ...
    this(
        ...
        IndexedProvider.from(List.of(buildText(size, DEFAULT_REGEXES, "0123456789".substring(0, Math.min(symbols, 10)), 100, new Random(seed)))),
        IndexedProvider.from(List.of(buildText(size, DEFAULT_REGEXES, "0123456789".substring(0, Math.min(symbols, 10)), 100, new Random(seed+1))))
    );
  }
